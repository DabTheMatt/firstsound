...
import { isDelayStereo } from '../../audio/fx/spaceModel'
...
export function SpaceInspector({ snap, kind, variant, pane }: Props) {
  ...
  const stereo = isDelayStereo(snap.params)
  ...
  return pane === 'advanced' ? (
    <>
      <details className={styles.band} open>
        ...
        {kind === 'delay' ? (
          <>
            <p className={styles.help}>{stereo ? 'Left and right can sync to different notes.' : 'One time for both channels.'}</p>
            <SyncRow snap={snap} label={stereo ? 'Left' : 'Delay'} syncId="delaySync" noteId="delayNote" kindId="delayNoteKind" />
            {stereo ? (
    ...
    <>
      ...
      <p className={styles.help}>
        Categories load a starting sound. Delay type sets analog / tape / digital tone (filters, drive,
