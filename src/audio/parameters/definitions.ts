import type { FilterType, ParamDef, ParamId, PlaybackDirection, StretchInterpAlgo } from './types'

export const PARAMS: Record<ParamId, ParamDef> = {
  start: {
    id: 'start',
    label: 'Start',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: 's',
    mapping: 'linear',
  },
  end: {
    id: 'end',
    label: 'End',
    min: 0,
    max: 1,
    defaultValue: 1,
    unit: 's',
    mapping: 'linear',
  },
  speed: {
    id: 'speed',
    label: 'Speed',
    min: 0.05,
    max: 8,
    defaultValue: 1,
    unit: 'x',
    mapping: 'log',
  },
  pitch: {
    id: 'pitch',
    label: 'Pitch',
    min: -48,
    max: 48,
    defaultValue: 0,
    unit: 'st',
    mapping: 'linear',
    step: 0.01,
  },
  stretchInterp: {
    id: 'stretchInterp',
    label: 'Overlap',
    min: 0,
    max: 100,
    defaultValue: 62,
    unit: '%',
    mapping: 'linear',
    step: 1,
  },
  stretchInterpOn: {
    id: 'stretchInterpOn',
    label: 'Interpolation',
    min: 0,
    max: 1,
    defaultValue: 1,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  stretchInterpAlgo: {
    id: 'stretchInterpAlgo',
    label: 'Interp Algo',
    min: 0,
    max: 3,
    defaultValue: 2,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  gain: {
    id: 'gain',
    label: 'Gain',
    min: -36,
    max: 24,
    defaultValue: -3,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  pan: {
    id: 'pan',
    label: 'Pan',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
    step: 1,
  },
  channelGainL: {
    id: 'channelGainL',
    label: 'Balance L',
    min: -24,
    max: 6,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  channelGainR: {
    id: 'channelGainR',
    label: 'Balance R',
    min: -24,
    max: 6,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  makeMono: {
    id: 'makeMono',
    label: 'Mono',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  invertPhase: {
    id: 'invertPhase',
    label: 'Invert Phase',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  grainSize: {
    id: 'grainSize',
    label: 'Grain Size',
    min: 8,
    max: 800,
    defaultValue: 120,
    unit: 'ms',
    mapping: 'log',
  },
  density: {
    id: 'density',
    label: 'Density',
    min: 1,
    max: 80,
    defaultValue: 18.4,
    unit: 'Hz',
    mapping: 'log',
  },
  position: {
    id: 'position',
    label: 'Position',
    min: 0,
    max: 100,
    defaultValue: 32,
    unit: '%',
    mapping: 'linear',
  },
  scatter: {
    id: 'scatter',
    label: 'Scatter',
    min: 0,
    max: 100,
    defaultValue: 45,
    unit: '%',
    mapping: 'linear',
  },
  grainPitch: {
    id: 'grainPitch',
    label: 'Pitch',
    min: -24,
    max: 24,
    defaultValue: 0,
    unit: 'st',
    mapping: 'linear',
    step: 0.01,
  },
  pitchSpread: {
    id: 'pitchSpread',
    label: 'Pitch Spread',
    min: 0,
    max: 24,
    defaultValue: 7.2,
    unit: 'st',
    mapping: 'linear',
    step: 0.01,
  },
  filterCutoff: {
    id: 'filterCutoff',
    label: 'Cutoff',
    min: 20,
    max: 20000,
    defaultValue: 1200,
    unit: 'Hz',
    mapping: 'log',
  },
  filterReso: {
    id: 'filterReso',
    label: 'Resonance',
    min: 0.1,
    max: 24,
    defaultValue: 0.85,
    unit: 'Q',
    mapping: 'log',
    step: 0.01,
  },
  filterDrive: {
    id: 'filterDrive',
    label: 'Drive',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  filterMix: {
    id: 'filterMix',
    label: 'Mix',
    min: 0,
    max: 100,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
  },
  filterKind: {
    id: 'filterKind',
    label: 'Type',
    min: 0,
    max: 6,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  filterSlope: {
    id: 'filterSlope',
    label: 'Slope',
    min: 0,
    max: 5,
    defaultValue: 1,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  filterCharacter: {
    id: 'filterCharacter',
    label: 'Character',
    min: 0,
    max: 4,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  filterMorph: {
    id: 'filterMorph',
    label: 'Morph',
    min: 0,
    max: 100,
    defaultValue: 50,
    unit: '%',
    mapping: 'linear',
  },
  filterLfoRate: {
    id: 'filterLfoRate',
    label: 'Rate',
    min: 0.01,
    max: 30,
    defaultValue: 0.4,
    unit: 'Hz',
    mapping: 'log',
  },
  filterLfoDepth: {
    id: 'filterLfoDepth',
    label: 'Depth',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  filterLfoShape: {
    id: 'filterLfoShape',
    label: 'Shape',
    min: 0,
    max: 5,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  filterLfoSync: {
    id: 'filterLfoSync',
    label: 'Sync',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  filterLfoNote: {
    id: 'filterLfoNote',
    label: 'Note',
    min: 0,
    max: 7,
    defaultValue: 4,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  filterLfoNoteKind: {
    id: 'filterLfoNoteKind',
    label: 'Feel',
    min: 0,
    max: 2,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  filterEnvAmt: {
    id: 'filterEnvAmt',
    label: 'Env Amt',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  filterEnvAttack: {
    id: 'filterEnvAttack',
    label: 'Env Attack',
    min: 0.5,
    max: 200,
    defaultValue: 8,
    unit: 'ms',
    mapping: 'log',
  },
  filterEnvRelease: {
    id: 'filterEnvRelease',
    label: 'Env Release',
    min: 5,
    max: 2000,
    defaultValue: 140,
    unit: 'ms',
    mapping: 'log',
  },
  filterEnvDir: {
    id: 'filterEnvDir',
    label: 'Direction',
    min: 0,
    max: 1,
    defaultValue: 1,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  filterAdsAmt: {
    id: 'filterAdsAmt',
    label: 'Env Amount',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  filterAdsAttack: {
    id: 'filterAdsAttack',
    label: 'Attack',
    min: 1,
    max: 2000,
    defaultValue: 12,
    unit: 'ms',
    mapping: 'log',
  },
  filterAdsDecay: {
    id: 'filterAdsDecay',
    label: 'Decay',
    min: 5,
    max: 4000,
    defaultValue: 180,
    unit: 'ms',
    mapping: 'log',
  },
  filterAdsSustain: {
    id: 'filterAdsSustain',
    label: 'Sustain',
    min: 0,
    max: 100,
    defaultValue: 35,
    unit: '%',
    mapping: 'linear',
  },
  filterAdsRelease: {
    id: 'filterAdsRelease',
    label: 'Release',
    min: 10,
    max: 4000,
    defaultValue: 220,
    unit: 'ms',
    mapping: 'log',
  },
  filterPitchTrack: {
    id: 'filterPitchTrack',
    label: 'Pitch Track',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayDry: {
    id: 'delayDry',
    label: 'Dry',
    min: 0,
    max: 100,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
  },
  delayDryR: {
    id: 'delayDryR',
    label: 'Dry R',
    min: 0,
    max: 100,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
  },
  delayWet: {
    id: 'delayWet',
    label: 'Wet',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayWetR: {
    id: 'delayWetR',
    label: 'Wet R',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayCorrelate: {
    id: 'delayCorrelate',
    label: 'Correlate',
    min: 0,
    max: 1,
    defaultValue: 1,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  delayOutput: {
    id: 'delayOutput',
    label: 'Output',
    min: 0,
    max: 200,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
  },
  delayTime: {
    id: 'delayTime',
    label: 'Time',
    min: 1,
    max: 10000,
    defaultValue: 300,
    unit: 'ms',
    mapping: 'log',
  },
  delayTimeR: {
    id: 'delayTimeR',
    label: 'Time R',
    min: 1,
    max: 10000,
    defaultValue: 300,
    unit: 'ms',
    mapping: 'log',
  },
  delayStereo: {
    id: 'delayStereo',
    label: 'Stereo',
    min: 0,
    max: 1,
    defaultValue: 1,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  delayFeedback: {
    id: 'delayFeedback',
    label: 'Feedback',
    min: 0,
    max: 95,
    defaultValue: 28,
    unit: '%',
    mapping: 'linear',
  },
  delayFeedbackR: {
    id: 'delayFeedbackR',
    label: 'Feedback R',
    min: 0,
    max: 95,
    defaultValue: 28,
    unit: '%',
    mapping: 'linear',
  },
  reverbDry: {
    id: 'reverbDry',
    label: 'Dry',
    min: 0,
    max: 100,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
  },
  reverbWet: {
    id: 'reverbWet',
    label: 'Wet',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  reverbCorrelate: {
    id: 'reverbCorrelate',
    label: 'Correlate',
    min: 0,
    max: 1,
    defaultValue: 1,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  reverbOutput: {
    id: 'reverbOutput',
    label: 'Output',
    min: 0,
    max: 200,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
  },
  reverbLimit: {
    id: 'reverbLimit',
    label: 'Limit',
    min: 0,
    max: 24,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.5,
  },
  motionDepth: {
    id: 'motionDepth',
    label: 'Depth',
    min: 0,
    max: 100,
    // Off by default so the grain position stays where you set it.
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  motionRate: {
    id: 'motionRate',
    label: 'Rate',
    min: 0.02,
    max: 8,
    defaultValue: 0.3,
    unit: 'Hz',
    mapping: 'log',
  },
  motionJitter: {
    id: 'motionJitter',
    label: 'Jitter',
    min: 0,
    max: 100,
    defaultValue: 30,
    unit: '%',
    mapping: 'linear',
  },
  outputGain: {
    id: 'outputGain',
    label: 'Output Gain',
    min: -24,
    max: 6,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  saturation: {
    id: 'saturation',
    label: 'Drive',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  saturationMix: {
    id: 'saturationMix',
    label: 'Mix',
    min: 0,
    max: 100,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
  },
  distortionTone: {
    id: 'distortionTone',
    label: 'Tone',
    min: 0,
    max: 100,
    defaultValue: 58,
    unit: '%',
    mapping: 'linear',
  },
  distortionBias: {
    id: 'distortionBias',
    label: 'Bias',
    min: 0,
    max: 100,
    defaultValue: 50,
    unit: '%',
    mapping: 'linear',
  },
  distortionBits: {
    id: 'distortionBits',
    label: 'Bits',
    min: 1,
    max: 16,
    defaultValue: 16,
    unit: 'bit',
    mapping: 'linear',
    step: 1,
  },
  distortionDownsample: {
    id: 'distortionDownsample',
    label: 'Rate',
    min: 1,
    max: 64,
    defaultValue: 1,
    unit: '×',
    mapping: 'linear',
    step: 1,
  },
  distortionNoise: {
    id: 'distortionNoise',
    label: 'Noise',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  distortionOutput: {
    id: 'distortionOutput',
    label: 'Output',
    min: -24,
    max: 12,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  reverbSize: {
    id: 'reverbSize',
    label: 'Size',
    min: 0,
    max: 100,
    defaultValue: 50,
    unit: '%',
    mapping: 'linear',
  },
  reverbDecay: {
    id: 'reverbDecay',
    label: 'Decay',
    min: 0.05,
    max: 60,
    defaultValue: 1.6,
    unit: 's',
    mapping: 'log',
  },
  reverbPredelay: {
    id: 'reverbPredelay',
    label: 'Pre-Delay',
    min: 0.1,
    max: 1000,
    defaultValue: 18,
    unit: 'ms',
    mapping: 'log',
  },
  reverbDamping: {
    id: 'reverbDamping',
    label: 'High Damp',
    min: 200,
    max: 18000,
    defaultValue: 7000,
    unit: 'Hz',
    mapping: 'log',
  },
  bpm: {
    id: 'bpm',
    label: 'Tempo',
    min: 40,
    max: 240,
    defaultValue: 120,
    unit: 'bpm',
    mapping: 'linear',
    step: 0.1,
  },
  delaySync: {
    id: 'delaySync',
    label: 'Sync',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  delayNote: {
    id: 'delayNote',
    label: 'Note',
    min: 0,
    max: 7,
    defaultValue: 4,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  delayNoteKind: {
    id: 'delayNoteKind',
    label: 'Feel',
    min: 0,
    max: 2,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  delaySyncR: {
    id: 'delaySyncR',
    label: 'Sync R',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  delayNoteR: {
    id: 'delayNoteR',
    label: 'Note R',
    min: 0,
    max: 7,
    defaultValue: 4,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  delayNoteKindR: {
    id: 'delayNoteKindR',
    label: 'Feel R',
    min: 0,
    max: 2,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  delayWidth: {
    id: 'delayWidth',
    label: 'Width',
    min: 0,
    max: 200,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
  },
  delayPan: {
    id: 'delayPan',
    label: 'Pan',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayOffset: {
    id: 'delayOffset',
    label: 'L/R Offset',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayHp: {
    id: 'delayHp',
    label: 'HP',
    min: 20,
    max: 10000,
    defaultValue: 40,
    unit: 'Hz',
    mapping: 'log',
  },
  delayLp: {
    id: 'delayLp',
    label: 'LP',
    min: 200,
    max: 20000,
    defaultValue: 10000,
    unit: 'Hz',
    mapping: 'log',
  },
  delayDrive: {
    id: 'delayDrive',
    label: 'Drive',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayModRate: {
    id: 'delayModRate',
    label: 'Mod Rate',
    min: 0.01,
    max: 20,
    defaultValue: 0.4,
    unit: 'Hz',
    mapping: 'log',
  },
  delayModDepth: {
    id: 'delayModDepth',
    label: 'Mod Depth',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayWow: {
    id: 'delayWow',
    label: 'Wow',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayFlutter: {
    id: 'delayFlutter',
    label: 'Flutter',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayDiffusion: {
    id: 'delayDiffusion',
    label: 'Diffusion',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayPitch: {
    id: 'delayPitch',
    label: 'Pitch',
    min: -48,
    max: 48,
    defaultValue: 0,
    unit: 'st',
    mapping: 'linear',
    step: 0.01,
  },
  delayReverse: {
    id: 'delayReverse',
    label: 'Reverse',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayDuck: {
    id: 'delayDuck',
    label: 'Duck',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  delayFreeze: {
    id: 'delayFreeze',
    label: 'Freeze',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  delayDrift: {
    id: 'delayDrift',
    label: 'Drift',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  reverbSync: {
    id: 'reverbSync',
    label: 'Sync',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  reverbNote: {
    id: 'reverbNote',
    label: 'Note',
    min: 0,
    max: 7,
    defaultValue: 4,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  reverbNoteKind: {
    id: 'reverbNoteKind',
    label: 'Feel',
    min: 0,
    max: 2,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  reverbEarly: {
    id: 'reverbEarly',
    label: 'Early',
    min: 0,
    max: 100,
    defaultValue: 40,
    unit: '%',
    mapping: 'linear',
  },
  reverbDiffusion: {
    id: 'reverbDiffusion',
    label: 'Diffusion',
    min: 0,
    max: 100,
    defaultValue: 55,
    unit: '%',
    mapping: 'linear',
  },
  reverbDensity: {
    id: 'reverbDensity',
    label: 'Density',
    min: 0,
    max: 100,
    defaultValue: 70,
    unit: '%',
    mapping: 'linear',
  },
  reverbLowCut: {
    id: 'reverbLowCut',
    label: 'Low Cut',
    min: 20,
    max: 2000,
    defaultValue: 20,
    unit: 'Hz',
    mapping: 'log',
  },
  reverbHighCut: {
    id: 'reverbHighCut',
    label: 'High Cut',
    min: 1000,
    max: 20000,
    defaultValue: 16000,
    unit: 'Hz',
    mapping: 'log',
  },
  reverbWidth: {
    id: 'reverbWidth',
    label: 'Width',
    min: 0,
    max: 200,
    defaultValue: 125,
    unit: '%',
    mapping: 'linear',
  },
  reverbPan: {
    id: 'reverbPan',
    label: 'Pan',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  reverbOffset: {
    id: 'reverbOffset',
    label: 'L/R Offset',
    min: -100,
    max: 100,
    defaultValue: 18,
    unit: '%',
    mapping: 'linear',
  },
  reverbInput: {
    id: 'reverbInput',
    label: 'Stereo In',
    min: 0,
    max: 100,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
  },
  reverbStereo: {
    id: 'reverbStereo',
    label: 'Stereo',
    min: 0,
    max: 1,
    defaultValue: 1,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  reverbModRate: {
    id: 'reverbModRate',
    label: 'Mod Rate',
    min: 0.01,
    max: 12,
    defaultValue: 0.35,
    unit: 'Hz',
    mapping: 'log',
  },
  reverbModDepth: {
    id: 'reverbModDepth',
    label: 'Mod Depth',
    min: 0,
    max: 100,
    defaultValue: 14,
    unit: '%',
    mapping: 'linear',
  },
  reverbShimmerPitch: {
    id: 'reverbShimmerPitch',
    label: 'Shimmer Pitch',
    min: -24,
    max: 24,
    defaultValue: 12,
    unit: 'st',
    mapping: 'linear',
    step: 0.01,
  },
  reverbShimmer: {
    id: 'reverbShimmer',
    label: 'Shimmer',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  reverbDrive: {
    id: 'reverbDrive',
    label: 'Drive',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  reverbDuck: {
    id: 'reverbDuck',
    label: 'Duck',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  reverbGate: {
    id: 'reverbGate',
    label: 'Gate',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  reverbGateThres: {
    id: 'reverbGateThres',
    label: 'Threshold',
    min: -60,
    max: 0,
    defaultValue: -24,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  reverbGateAttack: {
    id: 'reverbGateAttack',
    label: 'Attack',
    min: 0.1,
    max: 50,
    defaultValue: 1,
    unit: 'ms',
    mapping: 'log',
  },
  reverbGateHold: {
    id: 'reverbGateHold',
    label: 'Hold',
    min: 1,
    max: 500,
    defaultValue: 80,
    unit: 'ms',
    mapping: 'log',
  },
  reverbGateRelease: {
    id: 'reverbGateRelease',
    label: 'Release',
    min: 10,
    max: 2000,
    defaultValue: 120,
    unit: 'ms',
    mapping: 'log',
  },
  reverbReverse: {
    id: 'reverbReverse',
    label: 'Reverse',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  reverbFreeze: {
    id: 'reverbFreeze',
    label: 'Freeze',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  reverbColor: {
    id: 'reverbColor',
    label: 'Color',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
  },
  reverbDistance: {
    id: 'reverbDistance',
    label: 'Distance',
    min: 0,
    max: 100,
    defaultValue: 35,
    unit: '%',
    mapping: 'linear',
  },
  limiterThreshold: {
    id: 'limiterThreshold',
    label: 'Threshold',
    min: -48,
    max: 0,
    defaultValue: -6,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  limiterCeiling: {
    id: 'limiterCeiling',
    label: 'Ceiling',
    min: -12,
    max: 0,
    defaultValue: -0.3,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  limiterRelease: {
    id: 'limiterRelease',
    label: 'Release',
    min: 10,
    max: 2000,
    defaultValue: 120,
    unit: 'ms',
    mapping: 'log',
  },
  limiterAttack: {
    id: 'limiterAttack',
    label: 'Attack',
    min: 0.1,
    max: 50,
    defaultValue: 3,
    unit: 'ms',
    mapping: 'log',
  },
  limiterKnee: {
    id: 'limiterKnee',
    label: 'Knee',
    min: 0,
    max: 40,
    defaultValue: 6,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  limiterRatio: {
    id: 'limiterRatio',
    label: 'Ratio',
    min: 1,
    max: 20,
    defaultValue: 12,
    unit: ':1',
    mapping: 'linear',
    step: 0.1,
  },
  limiterMakeup: {
    id: 'limiterMakeup',
    label: 'Makeup',
    min: -12,
    max: 24,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  limiterInput: {
    id: 'limiterInput',
    label: 'Input',
    min: -12,
    max: 24,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  limiterAutoMakeup: {
    id: 'limiterAutoMakeup',
    label: 'Auto Makeup',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  compressorThreshold: {
    id: 'compressorThreshold',
    label: 'Threshold',
    min: -48,
    max: 0,
    defaultValue: -6,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  compressorRatio: {
    id: 'compressorRatio',
    label: 'Ratio',
    min: 1,
    max: 20,
    defaultValue: 12,
    unit: ':1',
    mapping: 'linear',
    step: 0.1,
  },
  compressorKnee: {
    id: 'compressorKnee',
    label: 'Knee',
    min: 0,
    max: 40,
    defaultValue: 6,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  compressorAttack: {
    id: 'compressorAttack',
    label: 'Attack',
    min: 0.1,
    max: 50,
    defaultValue: 3,
    unit: 'ms',
    mapping: 'log',
  },
  compressorRelease: {
    id: 'compressorRelease',
    label: 'Release',
    min: 10,
    max: 2000,
    defaultValue: 120,
    unit: 'ms',
    mapping: 'log',
  },
  compressorMakeup: {
    id: 'compressorMakeup',
    label: 'Makeup',
    min: -12,
    max: 24,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  compressorInput: {
    id: 'compressorInput',
    label: 'Input',
    min: -12,
    max: 24,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  compressorAutoMakeup: {
    id: 'compressorAutoMakeup',
    label: 'Auto Makeup',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  eq1Freq: { id: 'eq1Freq', label: 'EQ 1 Freq', min: 10, max: 25000, defaultValue: 80, unit: 'Hz', mapping: 'log' },
  eq1Gain: { id: 'eq1Gain', label: 'EQ 1 Gain', min: -18, max: 18, defaultValue: 0, unit: 'dB', mapping: 'linear', step: 0.1 },
  eq1Q: { id: 'eq1Q', label: 'EQ 1 Q', min: 0.1, max: 20, defaultValue: 0.7, unit: '', mapping: 'log' },
  eq2Freq: { id: 'eq2Freq', label: 'EQ 2 Freq', min: 10, max: 25000, defaultValue: 400, unit: 'Hz', mapping: 'log' },
  eq2Gain: { id: 'eq2Gain', label: 'EQ 2 Gain', min: -18, max: 18, defaultValue: 0, unit: 'dB', mapping: 'linear', step: 0.1 },
  eq2Q: { id: 'eq2Q', label: 'EQ 2 Q', min: 0.1, max: 20, defaultValue: 1, unit: '', mapping: 'log' },
  eq3Freq: { id: 'eq3Freq', label: 'EQ 3 Freq', min: 10, max: 25000, defaultValue: 2500, unit: 'Hz', mapping: 'log' },
  eq3Gain: { id: 'eq3Gain', label: 'EQ 3 Gain', min: -18, max: 18, defaultValue: 0, unit: 'dB', mapping: 'linear', step: 0.1 },
  eq3Q: { id: 'eq3Q', label: 'EQ 3 Q', min: 0.1, max: 20, defaultValue: 1, unit: '', mapping: 'log' },
  eq4Freq: { id: 'eq4Freq', label: 'EQ 4 Freq', min: 10, max: 25000, defaultValue: 12000, unit: 'Hz', mapping: 'log' },
  eq4Gain: { id: 'eq4Gain', label: 'EQ 4 Gain', min: -18, max: 18, defaultValue: 0, unit: 'dB', mapping: 'linear', step: 0.1 },
  eq4Q: { id: 'eq4Q', label: 'EQ 4 Q', min: 0.1, max: 20, defaultValue: 0.7, unit: '', mapping: 'log' },
  eq5Freq: { id: 'eq5Freq', label: 'EQ 5 Freq', min: 10, max: 25000, defaultValue: 160, unit: 'Hz', mapping: 'log' },
  eq5Gain: { id: 'eq5Gain', label: 'EQ 5 Gain', min: -18, max: 18, defaultValue: 0, unit: 'dB', mapping: 'linear', step: 0.1 },
  eq5Q: { id: 'eq5Q', label: 'EQ 5 Q', min: 0.1, max: 20, defaultValue: 1, unit: '', mapping: 'log' },
  eq6Freq: { id: 'eq6Freq', label: 'EQ 6 Freq', min: 10, max: 25000, defaultValue: 800, unit: 'Hz', mapping: 'log' },
  eq6Gain: { id: 'eq6Gain', label: 'EQ 6 Gain', min: -18, max: 18, defaultValue: 0, unit: 'dB', mapping: 'linear', step: 0.1 },
  eq6Q: { id: 'eq6Q', label: 'EQ 6 Q', min: 0.1, max: 20, defaultValue: 1, unit: '', mapping: 'log' },
  eq7Freq: { id: 'eq7Freq', label: 'EQ 7 Freq', min: 10, max: 25000, defaultValue: 5000, unit: 'Hz', mapping: 'log' },
  eq7Gain: { id: 'eq7Gain', label: 'EQ 7 Gain', min: -18, max: 18, defaultValue: 0, unit: 'dB', mapping: 'linear', step: 0.1 },
  eq7Q: { id: 'eq7Q', label: 'EQ 7 Q', min: 0.1, max: 20, defaultValue: 1, unit: '', mapping: 'log' },
  eq8Freq: { id: 'eq8Freq', label: 'EQ 8 Freq', min: 10, max: 25000, defaultValue: 16000, unit: 'Hz', mapping: 'log' },
  eq8Gain: { id: 'eq8Gain', label: 'EQ 8 Gain', min: -18, max: 18, defaultValue: 0, unit: 'dB', mapping: 'linear', step: 0.1 },
  eq8Q: { id: 'eq8Q', label: 'EQ 8 Q', min: 0.1, max: 20, defaultValue: 0.7, unit: '', mapping: 'log' },
  eqcfTeeth: { id: 'eqcfTeeth', label: 'Comb Teeth', min: 2, max: 16, defaultValue: 5, unit: '', mapping: 'linear', step: 1 },
  eqcfGain: { id: 'eqcfGain', label: 'Comb Gain', min: -18, max: 18, defaultValue: 6, unit: 'dB', mapping: 'linear', step: 0.1 },
  eqcfSpacing: { id: 'eqcfSpacing', label: 'Comb Spacing', min: 10, max: 4000, defaultValue: 200, unit: 'Hz', mapping: 'log' },
  eqcfFreq: { id: 'eqcfFreq', label: 'Comb Base', min: 10, max: 25000, defaultValue: 110, unit: 'Hz', mapping: 'log' },
  msWidth: {
    id: 'msWidth',
    label: 'Width',
    min: 0,
    max: 200,
    defaultValue: 100,
    unit: '%',
    mapping: 'linear',
    step: 0.1,
  },
  msBalance: {
    id: 'msBalance',
    label: 'M/S Balance',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
    step: 0.1,
  },
  msMidGain: {
    id: 'msMidGain',
    label: 'Mid Gain',
    min: -60,
    max: 12,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  msSideGain: {
    id: 'msSideGain',
    label: 'Side Gain',
    min: -60,
    max: 12,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  msSideHpf: {
    id: 'msSideHpf',
    label: 'Center Below',
    min: 0,
    max: 1000,
    defaultValue: 0,
    unit: 'Hz',
    mapping: 'linear',
    step: 1,
  },
  msMidTilt: {
    id: 'msMidTilt',
    label: 'Mid Tilt',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
    step: 1,
  },
  msSideTilt: {
    id: 'msSideTilt',
    label: 'Side Tilt',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
    step: 1,
  },
  msMidLowFreq: {
    id: 'msMidLowFreq',
    label: 'Mid Low Freq',
    min: 20,
    max: 500,
    defaultValue: 120,
    unit: 'Hz',
    mapping: 'log',
  },
  msMidLowGain: {
    id: 'msMidLowGain',
    label: 'Mid Low',
    min: -18,
    max: 18,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  msMidPeakFreq: {
    id: 'msMidPeakFreq',
    label: 'Mid Bell Freq',
    min: 200,
    max: 8000,
    defaultValue: 1200,
    unit: 'Hz',
    mapping: 'log',
  },
  msMidPeakGain: {
    id: 'msMidPeakGain',
    label: 'Mid Bell',
    min: -18,
    max: 18,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  msMidPeakQ: {
    id: 'msMidPeakQ',
    label: 'Mid Bell Q',
    min: 0.3,
    max: 8,
    defaultValue: 1,
    unit: '',
    mapping: 'log',
  },
  msMidHighFreq: {
    id: 'msMidHighFreq',
    label: 'Mid High Freq',
    min: 2000,
    max: 16000,
    defaultValue: 8000,
    unit: 'Hz',
    mapping: 'log',
  },
  msMidHighGain: {
    id: 'msMidHighGain',
    label: 'Mid High',
    min: -18,
    max: 18,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  msSideLowFreq: {
    id: 'msSideLowFreq',
    label: 'Side Low Freq',
    min: 20,
    max: 500,
    defaultValue: 120,
    unit: 'Hz',
    mapping: 'log',
  },
  msSideLowGain: {
    id: 'msSideLowGain',
    label: 'Side Low',
    min: -18,
    max: 18,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  msSidePeakFreq: {
    id: 'msSidePeakFreq',
    label: 'Side Bell Freq',
    min: 200,
    max: 8000,
    defaultValue: 1200,
    unit: 'Hz',
    mapping: 'log',
  },
  msSidePeakGain: {
    id: 'msSidePeakGain',
    label: 'Side Bell',
    min: -18,
    max: 18,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  msSidePeakQ: {
    id: 'msSidePeakQ',
    label: 'Side Bell Q',
    min: 0.3,
    max: 8,
    defaultValue: 1,
    unit: '',
    mapping: 'log',
  },
  msSideHighFreq: {
    id: 'msSideHighFreq',
    label: 'Side High Freq',
    min: 2000,
    max: 16000,
    defaultValue: 8000,
    unit: 'Hz',
    mapping: 'log',
  },
  msSideHighGain: {
    id: 'msSideHighGain',
    label: 'Side High',
    min: -18,
    max: 18,
    defaultValue: 0,
    unit: 'dB',
    mapping: 'linear',
    step: 0.1,
  },
  msRotate: {
    id: 'msRotate',
    label: 'Rotate',
    min: -100,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
    step: 0.1,
  },
  msCrossfeed: {
    id: 'msCrossfeed',
    label: 'Crossfeed',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
    step: 0.1,
  },
  msHaasTime: {
    id: 'msHaasTime',
    label: 'Micro Time',
    min: 0,
    max: 30,
    defaultValue: 8,
    unit: 'ms',
    mapping: 'linear',
    step: 0.1,
  },
  msHaasAmount: {
    id: 'msHaasAmount',
    label: 'Micro Amount',
    min: 0,
    max: 100,
    defaultValue: 0,
    unit: '%',
    mapping: 'linear',
    step: 0.1,
  },
  msHaasDir: {
    id: 'msHaasDir',
    label: 'Micro Dir',
    min: 0,
    max: 1,
    defaultValue: 1,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  msSoloMid: {
    id: 'msSoloMid',
    label: 'M Solo',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  msSoloSide: {
    id: 'msSoloSide',
    label: 'S Solo',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  msMono: {
    id: 'msMono',
    label: 'Mono',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  msFlipMid: {
    id: 'msFlipMid',
    label: 'Ø Mid',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
  msFlipSide: {
    id: 'msFlipSide',
    label: 'Ø Side',
    min: 0,
    max: 1,
    defaultValue: 0,
    unit: '',
    mapping: 'linear',
    step: 1,
  },
}

/** Filter type options, in UI order. `off` bypasses the filter entirely. */
export const FILTER_TYPES: { value: FilterType; label: string }[] = [
  { value: 'off', label: 'Off' },
  { value: 'lowpass', label: 'Low Pass' },
  { value: 'highpass', label: 'High Pass' },
  { value: 'lowshelf', label: 'Low Shelf' },
  { value: 'highshelf', label: 'High Shelf' },
  { value: 'peaking', label: 'Bell' },
  { value: 'notch', label: 'Notch' },
  { value: 'bandpass', label: 'Band Pass' },
]

/** Playback direction options for the region player. */
export const PLAYBACK_DIRECTIONS: { value: PlaybackDirection; label: string }[] = [
  { value: 'forward', label: 'Forward' },
  { value: 'reverse', label: 'Reverse' },
  { value: 'pingpong', label: 'Ping-Pong' },
]

export const PARAM_IDS = Object.keys(PARAMS) as ParamId[]

export const SOURCE_KNOBS: ParamId[] = ['speed', 'pitch', 'stretchInterp', 'gain', 'pan']

export const STRETCH_INTERP_ALGOS: { value: StretchInterpAlgo; label: string; title: string }[] = [
  { value: 'nearest', label: 'Nearest', title: 'Drop-sample, no reconstruction' },
  { value: 'linear', label: 'Linear', title: 'Two-point lerp' },
  { value: 'cubic', label: 'Cubic', title: '4-point Hermite, fuller lows' },
  { value: 'sinc', label: 'Sinc', title: 'Windowed sinc, band-limited' },
]
export const GRAIN_KNOBS: ParamId[] = [
  'grainSize',
  'density',
  'position',
  'scatter',
  'grainPitch',
  'pitchSpread',
]
export const MOTION_KNOBS: ParamId[] = ['motionDepth', 'motionRate', 'motionJitter', 'position']
export const SPACE_KNOBS: ParamId[] = [
  'delayDry',
  'delayWet',
  'delayTime',
  'delayFeedback',
  'reverbDry',
  'reverbWet',
  'reverbSize',
  'reverbDecay',
  'reverbWidth',
]
export const DELAY_MAIN_KNOBS: ParamId[] = ['delayDry', 'delayWet', 'delayTime', 'delayFeedback']
export const REVERB_MAIN_KNOBS: ParamId[] = [
  'reverbDry',
  'reverbWet',
  'reverbSize',
  'reverbDecay',
  'reverbWidth',
  'reverbLowCut',
  'reverbHighCut',
  'reverbDamping',
]
export const FILTER_KNOBS: ParamId[] = ['filterCutoff', 'filterReso', 'filterDrive', 'filterMix']
export const FILTER_MOD_KNOBS: ParamId[] = [
  'filterLfoRate',
  'filterLfoDepth',
  'filterEnvAmt',
  'filterAdsAmt',
]
export const COMPRESSOR_MAIN_KNOBS: ParamId[] = [
  'compressorThreshold',
  'compressorRatio',
  'compressorKnee',
  'compressorRelease',
]
export const COMPRESSOR_ADV_KNOBS: ParamId[] = [
  'compressorInput',
  'compressorAttack',
  'compressorMakeup',
]
export const LIMITER_MAIN_KNOBS: ParamId[] = ['limiterCeiling', 'limiterRelease']
export const LIMITER_ADV_KNOBS: ParamId[] = ['limiterInput', 'limiterAttack']
export const DISTORTION_MAIN_KNOBS: ParamId[] = ['saturation', 'saturationMix']
export const DISTORTION_ADV_KNOBS: ParamId[] = [
  'distortionTone',
  'distortionBias',
  'distortionBits',
  'distortionDownsample',
  'distortionNoise',
  'distortionOutput',
]
export const MIDSIDE_MAIN_KNOBS: ParamId[] = [
  'msWidth',
  'msBalance',
  'msMidGain',
  'msSideGain',
  'msSideHpf',
  'msRotate',
  'msCrossfeed',
  'msMidLowGain',
  'msSideLowGain',
]
export const MIDSIDE_ADV_KNOBS: ParamId[] = [
  'msHaasTime',
  'msHaasAmount',
  'msMidTilt',
  'msSideTilt',
]
export const OUTPUT_KNOBS: ParamId[] = ['gain']

export function defaultParamValues(): Record<ParamId, number> {
  const values = {} as Record<ParamId, number>
  for (const id of PARAM_IDS) {
    values[id] = PARAMS[id].defaultValue
  }
  return values
}
